...
#       * Add file load command to plpbridge
...
class debug_bridge(object):
    ...
    def load_file(self, file, addr):
        size = os.path.getsize(file)
        print('Loading file')
        print('Loading %d bytes at 0x%x' % (size, addr))
        f = open(file, "rb")
        img = f.read()
        self.write(addr, size, img)
        f.close()
        return 0
    ...
    def reqloop(self):
        ...
        print('Using Debug Struct at: __rt_debug_struct_ptr 0x%x' % (addr))
        if addr == 0:
            addr = self._get_binary_symbol_addr('debugStruct_ptr')
            print('Using Fallback Debug Struct at: debugStruct_ptr 0x%x' % (addr))
