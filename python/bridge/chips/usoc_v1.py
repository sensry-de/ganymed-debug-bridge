...
# Authors: Marcus Pietzsch, IPMS (marcus.pietzsch@ipms.fraunhofer.de)
...
class usoc_v1_debug_bridge(debug_bridge):
    ...
    def load_jtag(self):
        ...
        #print ('Starting from L2')
        #self.write(0x1A112000, 4, [0x80, 0x80, 0x00, 0x1c])
        #print ('Starting from ROM [in load]')
        #self.write(0x1A112000, 4, [0x80, 0x00, 0x00, 0x1a])
    ...
    def start(self):
        ...
        if self.start_cores:
            print ('Start Cores')
            print ('Starting from ROM [in start]')
            self.write(0x1A112000, 4, [0x80, 0x00, 0x00, 0x1d])
            # Unstall the FC so that it starts fetching instructions from the loaded binary
            self.write(0x1A110000, 4, [0, 0, 0, 0])
        else:
            #this is executed only if no image was loaded beforehand
            #it is to be used if a bootloader was preloaded
            #set PC
            print ('Starting from ROM [in start]')
            self.write(0x1A112000, 4, [0x80, 0x00, 0x00, 0x1d])
            #print ('Starting from L2 [in start]')
            #self.write(0x1A112000, 4, [0x80, 0x80, 0x00, 0x1c])
            # Unstall the FC so that it starts fetching instructions from the loaded binary
            self.write(0x1A110000, 4, [0, 0, 0, 0])
